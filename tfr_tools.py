...
def Saving_Batch_TFR(path,idxs,batchs,labels,current,total):
    ...
    print('成功写入至第%d/%d个文件，序号项类型：%s，数据项类型：%s ， 标签项类型：%s' % (current,total,idx.dtype,
...
if __name__=='__main__':
    ...
    mnist = input_data.read_data_sets(r'D:\python_workspace\tensorflow\MNIST', one_hot=True)

    # 读取mnist数据。
    images = mnist.train.images.reshape([-1,28,28,1])  # 55000x28x28x1
    labels = mnist.train.labels  # 55000x10
    idxs = np.arange(0,labels.shape[0])
    ...
    # 分批保存
    # for i in range(5):
    #     images_seg = images[int(i*11000):int((i+1)*11000)]
    #     labels_seg = labels[int(i*11000):int((i+1)*11000)]
    #     idxs_seg = idxs[int(i*11000):int((i+1)*11000)]
    #     Saving_Batch_TFR(savepath,idxs,images,labels,i,4)
